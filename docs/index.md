...
The provider authenticates using credentials-based session authentication (username/password) against the Jitsu Console API. All four configuration values can be set via environment variables.

## Schema

### Optional

- `console_url` (String, Sensitive) - Jitsu Console URL. Can also be set via `JITSU_CONSOLE_URL` env var.
- `username` (String, Sensitive) - Jitsu username for session authentication. Can also be set via `JITSU_USERNAME` env var.
- `password` (String, Sensitive) - Jitsu password for session authentication. Can also be set via `JITSU_PASSWORD` env var.
